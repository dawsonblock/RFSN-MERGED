...
# Compatibility shim for legacy orchestrators
def triage_failure(
    test_id: str,
    error_message: str,
    traceback: str = "",
    ...
    """
    Legacy wrapper to classify a single test failure.

    Older orchestrator code expects a function named `triage_failure` that
    accepts a test identifier, error message, traceback and repository path
    and returns a FailureClassification.  This helper constructs a minimal
    TestResult and uses FailureTriage to produce a classification.

    Parameters
    ----------
    test_id: str
        Identifier of the failed test (e.g. file::test_name)
    error_message: str
        Error message associated with the failure
    traceback: str, optional
        Full traceback string; if absent an empty string may be passed
    repo_path: str, optional
        Path to the repository under test; currently unused but kept for
        backward compatibility

    Returns
    -------
    FailureClassification
        The classification of the test failure
    """
    # Construct a minimal TestResult
    test = TestResult(
        test_id=test_id,
    ...
    # Baseline flag is False because this is a validation failure
    classification = triage.classify_failure(test, is_baseline=False)
    return classification
