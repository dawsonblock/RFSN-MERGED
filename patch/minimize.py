...
#
# Historically, callers imported `minimize_patch` directly from this module.
# That function was removed when patch minimization was refactored into the
# PatchMinimizer class.  Define a thin wrapper here to preserve that API.

def minimize_patch(patch: Patch, repo_dir: Path) -> Patch:
    """Minimize a single patch using the default PatchMinimizer.

    This function instantiates a ``PatchMinimizer`` and delegates to its
    ``minimize_patch`` method.  It is provided solely for backwards
    compatibility with code that imported this symbol from ``patch.minimize``.
    ...
    """
    minimizer = PatchMinimizer()
    return minimizer.minimize_patch(patch, repo_dir)
