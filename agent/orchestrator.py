...
from .gate import gate as canonical_gate
...
# Import the staged test runner.  The runner no longer exposes a
# TestStageConfig class; instead, run_staged_tests accepts parameters
# directly and returns a dict with baseline/validation/comparison results.
from runner.tests import run_staged_tests
...
class EpisodeOrchestrator:
    ...
    def _gate(
        ...
        """Validate proposal through canonical gate.

        Delegates to the unified gate implementation in `agent.gate`.  This
        enforces budgets, phase, file, diff and test policies.  The
        orchestrator does not embed its own policy logic.
        ...
        Returns:
            Gate decision
        """
        return canonical_gate(profile, state, proposal)
    ...
    def _exec_staged_tests(self, state: AgentState, proposal: Proposal) -> ExecResult:
        ...
        # The staged test runner is asynchronous and returns a dictionary
        # containing baseline/validation/comparison objects and a success flag.
        # Because this orchestrator is synchronous, synchronously run the async
        # function via asyncio.run.  This ensures that tests execute and results
        # are available immediately.
        import asyncio

        # Set environment variable with patch diff for incremental test selection.
        # If a current patch has been applied, extract its diff(s) and expose it
        # via `RFSN_PATCH_DIFF`.  This allows the staged test runner to run only
        # affected tests before running the full suite.  Clear the variable if
        # there is no current patch.
        diff_text = None
        if self.current_patches:
            try:
                # Concatenate all diff hunks into a single unified diff
                candidate = self.current_patches[0]
                patch_obj = getattr(candidate, 'patch', None)
                if patch_obj and getattr(patch_obj, 'diffs', None):
                    diff_text = "\n".join(patch_obj.diffs)
            except Exception:
                diff_text = None
        if diff_text:
            os.environ["RFSN_PATCH_DIFF"] = diff_text
        else:
            os.environ.pop("RFSN_PATCH_DIFF", None)
        ...
        baseline = staged_results.get("baseline")
        validation = staged_results.get("validation")
        comparison = staged_results.get("comparison", {})
        success = staged_results.get("success", False)

        # Derive regressions and fixed counts from comparison summary
        fixed_tests = comparison.get("fixed_tests", [])
        regression_tests = comparison.get("regression_tests", [])
        ...
        total_duration = 0
        if baseline:
            total_duration += getattr(baseline, "duration_ms", 0)
        if validation:
            total_duration += getattr(validation, "duration_ms", 0)

        if success:
            logger.info("Staged tests PASSED")
            return ExecResult(
                status="ok",
                summary=f"Staged tests passed (fixed={fixed_count}, no new regressions)",
                metrics={
                    "test_result": {
                        "passed": True,
                        "fixed_regressions": fixed_count,
                        "new_regressions": regression_count,
                        "duration": total_duration,
                    }
                },
            )
        else:
            logger.warning(f"Staged tests FAILED (new regressions: {regression_count})")
            ...
            return ExecResult(
                status="fail",
                summary=f"Staged tests failed (new regressions: {regression_count})",
                metrics={
                    "test_result": {
                        "passed": False,
                        "new_regressions": regression_count,
