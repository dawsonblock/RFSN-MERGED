...
# Import the canonical gate implementation.  This ensures that
# DeepSeek agent uses the same deterministic gate policies as the
# orchestrated agent loop.
from agent.gate import gate as canonical_gate
...
def gate(profile: Profile, state: AgentState, proposal: Proposal) -> GateDecision:
    """Validate a proposal using the canonical gate.

    This wrapper ensures the DeepSeek agent shares the same deterministic
    gate logic as the orchestrated loop by delegating to
    ``agent.gate.gate``.  No policy enforcement lives here.
    """
