...
async def run_staged_tests(
    ...
    # Determine incremental test selection when no explicit targets provided.
    # If target_tests is None and an environment variable `RFSN_PATCH_DIFF` is present,
    # compute a focused list of tests based on the patch diff.  This enables
    # incremental test selection by running only tests that are likely affected
    # by the changes, before the full suite.  The incremental selection logic
    # uses the rfsn_controller.incremental_testing module to map changed
    # source files to relevant tests.
    selected_tests = target_tests
    try:
        if selected_tests is None:
            diff_text = os.environ.get("RFSN_PATCH_DIFF")
            # If diff is provided and non-empty, compute incremental tests
            if diff_text:
                from rfsn_controller.incremental_testing import select_tests_for_patch
                selection = select_tests_for_patch(diff_text, repo_path)
                # Prioritize affected tests; limit to a reasonable number to avoid overloading
                # the validation stage. If no affected tests found, leave as None to run full suite.
                if selection.affected_tests:
                    selected_tests = selection.affected_tests[:10]
    except Exception:
        # Fall back silently if incremental selection fails
        selected_tests = target_tests

    # Run validation (after patch would be applied)
    validation = await runner.run_validation(selected_tests)
