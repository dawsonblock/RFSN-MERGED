...
# NOTE: CGW mode has been removed.  The `_run_cgw_mode` function remains as a
# stub for backward compatibility but will raise an error if invoked.
def _run_cgw_mode(args):  # pragma: no cover
    """CGW mode is no longer supported in the merged repository."""
    raise RuntimeError("CGW mode has been removed in this build")
...
def main() -> None:
    ...
    # CGW Serial Decision Mode removed
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
    ...
    # SWE-bench / upstream learner integration
    parser.add_argument(
        "--profile",
        default=None,
        help="Profile name to load (from profiles.yaml or RFSN_PROFILES_PATH)",
    )
    parser.add_argument(
        "--goal",
        default=None,
        help="Task-specific goal description (used by planner)",
    )
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Run verification after patch application (CI mode)",
    )
    args = parser.parse_args()


    # CGW mode removed: ignore any requests for CGW mode
